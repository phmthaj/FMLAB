...
def precompute_rotary_emb(dim, max_positions):
    ...
    if dim % 2 != 0:
        print("RoPE requires an even head dimension")
        return rope_cache
    ...
    angles = torch.einsum('t,f->tf', t, inv_freq)
...
def apply_rotary_emb(x, rope_cache):
    ...
    B, H, T, hs = x.shape
    if hs % 2 != 0:
        print("RoPE head size must be even")
        return rotated_x

    half = hs // 2
    cos_sin = rope_cache[:T].to(dtype=x.dtype, device=x.device)  # (T, half, 2)
    x_reshaped = x.reshape(B, H, T, half, 2)
    x_complex = torch.view_as_complex(x_reshaped)  # (B, H, T, half)
    rot_complex = torch.view_as_complex(cos_sin).unsqueeze(0).unsqueeze(0)  # (1,1,T,half)
    x_rotated = x_complex * rot_complex
    rotated_x = torch.view_as_real(x_rotated).reshape(B, H, T, hs)
