...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        trunc_max = min(max_len, len(doc))
        trunc_len = random.randint(4, max(4, trunc_max))
        truncated = doc[:trunc_len]
        n = len(truncated)
        masked_len = random.randint(1, max(1, n // 2))  # ~ n/4 on average
        start = random.randint(0, n - masked_len)
        prefix = truncated[:start]
        masked_content = truncated[start:start + masked_len]
        suffix = truncated[start + masked_len:]
        masked_string_core = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content
        total_len = self.block_size + 1
        pads_needed = total_len - len(masked_string_core)
        if pads_needed < 0:
            masked_string_core = masked_string_core[:total_len]
            pads_needed = 0
        masked_string = masked_string_core + (self.PAD_CHAR * pads_needed)
